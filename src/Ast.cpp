...
namespace AST {
    ...
    struct astNode* statement() {
        ...
        case Continue:
            ++pos;  // for CONTINUE
            match(While);  // Ensure it is followed by WHILE
    ...
    struct astNode* assignStatement() {
        const char* varName = TokenTable[pos].name;
        match(Identifier);
        match(Assign);
        struct astNode* expr = arithmeticExpression();
        return createNode(assign_node, "assign", createNode(id_node, varName, nullptr, nullptr), expr);
    }
    struct astNode* ifStatement() {
        ...
        // Allow the if body to be empty
        struct astNode* stmt = NULL;
        if (TokenTable[pos].type != Else && TokenTable[pos].type != Semicolon) {
            stmt = statement();
        }
        if (TokenTable[pos].type == Semicolon) {
            ++pos;  // Consume optional semicolon after if body
        }

        // Handle the else part, if present
        if (TokenTable[pos].type == Else) {
            ++pos;  // Consume ELSE token

            struct astNode* elseStmt = NULL;
            if (TokenTable[pos].type != Semicolon) {
                elseStmt = statement();
                match(Semicolon);
            }
            if (TokenTable[pos].type == Semicolon) {
                ++pos;  // Consume optional semicolon after else body
            }
   ...
   // }
    struct astNode* forStatement() {
        ...
        // Parse the initialization (e.g., i := 1)
        struct astNode* assign = assignStatement();

        // Parse the range (e.g., To or Downto and the expression)
        struct astNode* range;
        if (TokenTable[pos].type == To) {
            ++pos;  // for "To"
            struct astNode* expr = arithmeticExpression();
            range = createNode(to_node, "to", assign, expr);
        }
        else if (TokenTable[pos].type == Downto) {
            ++pos;  // for "Downto"
            struct astNode* expr = arithmeticExpression();
            range = createNode(downto_node, "downto", assign, expr);
        }
        else {
            printf("\nSyntax error in line %d: 'to' or 'downto' operator was expected, but '%s' token found.\n", TokenTable[pos].line, lexemeTypeName(TokenTable[pos].type));
            exit(1);
        }

        match(Do);  // Match the "DO" keyword

        // Parse all statements between "DO" and the semicolon (";")
        struct astNode* body = nullptr;
        struct astNode* last = nullptr;

        while (TokenTable[pos].type != Semicolon) {
            struct astNode* stmt = statement();

            // Append this statement to the body tree
            if (body == nullptr) {
                body = stmt;  // First statement
            }
            else {
                last = createNode(statement_node, "statement", body, stmt);
                body = last;
            }
        }

        match(Semicolon);  // Ensure the loop ends with a semicolon

        // Return the FOR node, combining the range and the body
        return createNode(for_node, "for", range, body);
    }
