...
namespace Parser {
    ...
    void ifStatement() {
        ...
        // Check if the if body is empty or parse a statement
        if (TokenTable[pos].type != Else && TokenTable[pos].type != Semicolon) {
            statement(); // Parse the if body
        }
        if (TokenTable[pos].type == Semicolon) {
            ++pos; // Consume optional semicolon after the if body
        }
        ...
        if (TokenTable[pos].type == Else) {
            ...
            // Check if the else body is empty or parse a statement
            if (TokenTable[pos].type != Semicolon) {
                statement(); // Parse the else body
                match(Semicolon);
            }
            if (TokenTable[pos].type == Semicolon) {
                ++pos; // Consume optional semicolon after the else body
            }
        }
    }
    ...
    void forStatement() {
        match(For);  // Match the "FOR" keyword

        assignStatement();  // Parse the initialization statement (e.g., i := 1)

        // Parse the range: TO or DOWNTO
        if (TokenTable[pos].type == To || TokenTable[pos].type == Downto) {
            ++pos;  // Advance past "TO" or "DOWNTO"
        }
        else {
            printf("\nSyntax error in line %d: TO or DOWNTO was expected, but '%s' token gained.\n", TokenTable[pos].line, lexemeTypeName(TokenTable[pos].type));
            fprintf(errorFile, "\nSyntax error in line %d: TO or DOWNTO was expected, but '%s' token gained.\n", TokenTable[pos].line, lexemeTypeName(TokenTable[pos].type));
            exit(1);
        }

        arithmeticExpression();  // Parse the range's limit (e.g., 10)

        match(Do);  // Match the "DO" keyword

        // Parse all statements between "DO" and ";"
        while (TokenTable[pos].type != Semicolon) {
            statement();  // Parse each statement in the loop body
        }
